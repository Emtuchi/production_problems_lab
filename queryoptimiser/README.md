...
•	Analogy: If your table is a book, an index is like the book’s index at the back—it lets you quickly find a topic instead of reading every page.
```
...
•	Example: Fetching 100 orders and their items → 1 query for orders + 100 queries for items.
...
•	Example: SELECT * FROM orders LEFT JOIN order_items ON orders.id = order_items.order_id WHERE orders.user_id = 42
```
